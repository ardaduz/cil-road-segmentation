...
def input_check():
    filtered_data = dict()
    if len(sys.argv) < 2:
        print("ERROR: Missing arguments for city to be used, example usage:")
        print("'python collect_data.py los-angeles dallas'")
        ...
        for city in sys.argv[1:]:
            if city not in all_data:
                print('ERROR:', city, 'is not found in the input_cities.csv, stopping execution!')
                return False, filtered_data
            else:
...
if __name__ == '__main__':
    ...
    google_api_key = "AIzaSyD5izvX-PGjnNEtDKOBM2kJpqhDxWi-qgE"
    ...
    for city_name in filtered_data:
        ...
        for i in range(0, n_images):
            ...
            satellite_image, road_image = get_one_data_pair(city=city, api_key=google_api_key, location=location,
                                                               size=image_size)
