...
class Prediction:
    def __init__(self, test_dataset, test_filenames, models, logdir, submission_filename, predict_on_different_orientations=True,
                 DEBUG_MODE=False):
        ...
        self.prediction_dir = os.path.join(self.logdir, "results")
...
if __name__ == "__main__":
    ### PLEASE SET THESE ###
    equivariant_prediction_enabled = True
    mobilenet_based_model_path = "mobilenetv2_based_model_best_checkpoint.hdf5"
    xception_based_two_pyramid_model_path = "xception_based_model_with_two_pyramids_best_checkpoint.hdf5"
    xception_based_many_pyramid_model_path = "xception_based_model_with_many_pyramids_best_checkpoint.hdf5"
    submission_filename = "mobilenet_xceptiontwo_xceptionmany_rotation_flip_ensemble_majority_sigmoid.csv"
    ...
    dataset = Dataset(DEBUG_MODE=False,
                      ...
                      predict_on_different_orientations=equivariant_prediction_enabled)

    train_dataset, validation_dataset, test_dataset = dataset.get_datasets()

    print("LOADING MODEL 1")
    model = models.load_model(xception_based_two_pyramid_model_path,
                              custom_objects={'root_mean_squared_error': LossesMetrics.root_mean_squared_error,
                                              'bce_dice_loss': LossesMetrics.bce_dice_loss,
                                              'dice_loss': LossesMetrics.dice_loss})
    model.save_weights("dummy_weights.hdf5")
    xceptiontwo_model = XceptionTwoSpatialPyramid(input_shape=(input_size, input_size, 3), optimizer=None)
    xceptiontwo_model = xceptiontwo_model.get_model()
    xceptiontwo_model.load_weights("dummy_weights.hdf5")

    print("LOADING MODEL 2")
    model = models.load_model(mobilenet_based_model_path, custom_objects={'root_mean_squared_error': LossesMetrics.root_mean_squared_error,
                                                                          'bce_dice_loss': LossesMetrics.bce_dice_loss,
                                                                          'dice_loss': LossesMetrics.dice_loss})
    model.save_weights("dummy_weights.hdf5")
    ...
    mobilenet_model.load_weights("dummy_weights.hdf5")

    print("LOADING MODEL 3")
    model = models.load_model(xception_based_many_pyramid_model_path,
                              custom_objects={'root_mean_squared_error': LossesMetrics.root_mean_squared_error,
                                              'bce_dice_loss': LossesMetrics.bce_dice_loss,
                                              'dice_loss': LossesMetrics.dice_loss})
    model.save_weights("dummy_weights.hdf5")
    xceptionmany_model = XceptionManySpatialPyramid(input_shape=(input_size, input_size, 3), optimizer=None)
    xceptionmany_model = xceptionmany_model.get_model()
    xceptionmany_model.load_weights("dummy_weights.hdf5")
    os.remove("dummy_weights.hdf5")

    predictor = Prediction(test_dataset=test_dataset,
                           test_filenames=dataset.x_test_filenames,
                           models=[xceptionmany_model, xceptiontwo_model, mobilenet_model],
                           logdir=".",
